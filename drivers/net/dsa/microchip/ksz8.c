...
static void ksz8463_irq_bus_sync_unlock(struct irq_data *d)
{
	struct ksz_irq *kirq  = irq_data_get_irq_chip_data(d);
...
static const struct irq_chip ksz8463_irq_chip = {
	...
	.irq_bus_sync_unlock	= ksz8463_irq_bus_sync_unlock,
};
...
static int ksz8463_girq_setup(struct ksz_device *dev)
{
	...
	girq->masked = 0;
	snprintf(girq->name, sizeof(girq->name), "ksz8463-global-irq");
