					...
					u8 index)
{
	...
	data = KSZ8463_NOTIFY_BIT |
	       ...
	       pin->index;
	reg = cfg_base + KSZ8463_PATTERN_OFF;
	ret = ksz_write16(dev, reg, data);
	if (ret)
		return ret;

	reg = cfg_base + KSZ8463_CYCLE_WIDTH_OFF;
	ret = ksz_write32(dev, reg, cycle_width_ns);
	if (ret)
		return ret;

	/* Set cycle count 0 - Infinite */
	reg = cfg_base + KSZ8463_CYCLE_CNT_OFF;
	ret = ksz_write16(dev, reg, 0);
	if (ret)
		return ret;

	data = (pulse_width_ns / 8);
	reg = cfg_base + KSZ8463_PULSE_WIDTH_OFF;
	ret = ksz_write16(dev, reg, data);
	if (ret)
		return ret;

	reg = cfg_base + KSZ8463_TARGET_NSEC;
	ret = ksz_write32(dev, reg, target_time->tv_nsec);
	if (ret)
		return ret;

	reg = cfg_base + KSZ8463_TARGET_SEC;
	ret = ksz_write32(dev, reg, target_time->tv_sec);
	if (ret)
		return ret;
...
static int ksz_ptp_update_time(struct ksz_device *dev,
			       ...
			       const struct timespec64 *ts)
{
	...
	ret = ksz_ptp_update_time(dev, ts);
	if (ret)
		goto unlock;
...
static int ksz_ptp_adjust_time(struct ksz_device *dev, s64 delta)
{
	const u16 *regs = dev->info->regs;
	s32 sec, nsec;
	u16 data16;
...
static int ksz8463_ptp_adjtime(struct ptp_clock_info *ptp, s64 delta)
{
	...
	ret = ksz_ptp_adjust_time(dev, delta);
	if (ret)
		goto unlock;
...
void ksz8463_ptp_set_caps(struct dsa_switch *ds)
{
	...
	ptp_data->caps.n_pins		= dev->info->num_gpios;
	ptp_data->caps.n_per_out	= dev->info->num_gpios;
}
...
int ksz_ptp_clock_register(struct dsa_switch *ds)
{
	...
	for (i = 0; i < dev->info->num_gpios; i++) {
				      ...
				      struct ksz_port *port, int hw_irq)
{
	...
	for (i = 0; i < 2; i++) {
		...
		ptpmsg_irq->num = irq_create_mapping(ptpirq->domain, hw_irq + i);
		if (!ptpmsg_irq->num)
